...
class analyzer():
    ...
    def lepton_property_register(self,process:str):
        ...
        if self.channel == 'ElectronMuon' and process =='DY':
            postfix_l1ID = '[self.tree.DY_l1_id]'
            postfix_l2ID = '[self.tree.DY_l2_id]'
        else:
            postfix_l1ID =''
            postfix_l2ID = ''
        self.l1p4.SetPtEtaPhiM(eval('self.tree.'+self.property_name[process+'_p4']['l1'][0]+postfix_l1ID),eval('self.tree.'+self.property_name[process+'_p4']['l1'][1]+postfix_l1ID),eval('self.tree.'+self.property_name[process+'_p4']['l1'][2]+postfix_l1ID),eval('self.tree.'+self.property_name[process+'_p4']['l1'][3]+postfix_l1ID))
        
        self.l2p4.SetPtEtaPhiM(eval('self.tree.'+self.property_name[process+'_p4']['l2'][0]+postfix_l2ID),eval('self.tree.'+self.property_name[process+'_p4']['l2'][1]+postfix_l2ID),eval('self.tree.'+self.property_name[process+'_p4']['l2'][2]+postfix_l2ID),eval('self.tree.'+self.property_name[process+'_p4']['l2'][3]+postfix_l2ID))
