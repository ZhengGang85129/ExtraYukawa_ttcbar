...
'''
h1_pucommand='''self.h1_{0}pu{1} = None'''
...
fill_histocommand='''
all_events1 = TH1F('self.all_events1','lep_tag',1,0,1)
all_events2 = TH1F('self.all_events2','met_tag',1,0,1)
all_events3 = TH1F('self.all_events3','lepmet_tag',1,0,1)
pass_lep_trigger= TH1F('self.pass_lep_trigger','pass_lep_trigger',1,0,1)
pass_met_trigger= TH1F('self.pass_met_trigger','pass_met_trigger',1,0,1)
pass_lepmet_trigger= TH1F('self.pass_lepmet_trigger','pass_lepmet_trigger',1,0,1)
for ientry in range(0,200000):
    ...
    if not (self.tree.Flag_goodVertices and self.tree.Flag_globalSuperTightHalo2016Filter and self.tree.Flag_HBHENoiseFilter and self.tree.Flag_HBHENoiseIsoFilter and self.tree.Flag_EcalDeadCellTriggerPrimitiveFilter and self.tree.Flag_BadPFMuonFilter and self.tree.Flag_eeBadScFilter and self.tree.Flag_ecalBadCalibFilter):
        ...
        if self.tree.DY_region == {0}:
            self.l1p4.SetPtEtaPhiM(self.tree.{1},self.tree.{2},self.tree.{3},{4})
        ...
        if({20}):
            ...
            if self.tree.n_tight_jet > 3 :
                ...
                self.h1_pre_l2_pt_highjet.Fill(self.l2p4.Pt(),weight)
                self.h1_pre_l2_eta_highjet.Fill(self.l2p4.Eta(),weight)
                ...
                self.h1_pre_l2_pt_lowjet.Fill(self.l2p4.Pt(),weight)
                self.h1_pre_l2_eta_lowjet.Fill(self.l2p4.Eta(),weight)
            ...
            if({19}):
                ...
                if self.tree.n_tight_jet > 3 :
                    ...
                    self.h1_l2_pt_highjet.Fill(self.l2p4.Pt(),weight)
                    self.h1_l2_eta_highjet.Fill(self.l2p4.Eta(),weight)
                    ...
                    self.h1_l2_pt_lowjet.Fill(self.l2p4.Pt(),weight)
                    self.h1_l2_eta_lowjet.Fill(self.l2p4.Eta(),weight)
    ...
    ##Histogram Write
    #for obj in ['lepmet','lep','met','njet','met','pu','putrue']: 
    #    exec(cmd_str.eff_trig_write_command.format(lep,obv,condition,obj))
...
Eff_{0}{1}{2}{3}.Write()
self.h2_{0}{1}{2}{3}.Divide(self.h2_pre_{0}{1}{2}{3})'''
eff_for_objcommand='''
Eff_{0}=TEfficiency(self.h1_{0}, self.h1_pre_{0})
Eff_{0}.SetTitle('Eff {0}')
Eff_{0}.Write()'''
eff_for_trigcommand='''
Eff_{0}trigger=TEfficiency(pass_{0}_trigger, all_events{1})
...
print '{0} trigger eff:', {0}eff,'+-',{0}eff_err
'''
...
eff_trig_write_command ='''Eff_{0}trigger.Write()'''
